  ...
  const handleClone = useCallback(async () => {
    ...
    const onProgress = (update: any) => {
        ...
        if (update.type === 'channel') {
            completedSteps++;
            setProgress(prev => ({ ...prev, channels: { ...prev.channels, current: update.current }, overall: Math.round((completedSteps / totalSteps) * 100) }));
        }
        if (update.type === 'role') {
            completedSteps++;
            setProgress(prev => ({ ...prev, roles: { ...prev.roles, current: update.current }, overall: Math.round((completedSteps / totalSteps) * 100) }));
        }
    };
